...
$document->addScriptdeclaration('jQuery(document).bind("ready",function(){jQuery("#user-registration").validate({wrapper:"mark"})});');
?>
<div class="page-reset page-reset__<?php echo $this->pageclass_sfx?>">
	...
	<?php endif; ?>
	<form id="user-registration" action="<?php echo JRoute::_('index.php?option=com_users&task=reset.request'); ?>" method="post" class="form-horizontal">
		<?php foreach ($this->form->getFieldsets() as $fieldset) : ?>
			<fieldset>
				<p><?php echo JText::_($fieldset->label); ?></p>
				<?php foreach ($this->form->getFieldset($fieldset->name) as $name => $field) : ?>
					<?php if($field->label != '' || $field->input != '') : ?>
