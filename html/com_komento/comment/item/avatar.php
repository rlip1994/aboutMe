...
>
	...
	<?php }
		$avatar = $row->author->getAvatar( $row->email );
		if(strpos($avatar,'d=mm')){
			//$avatar = str_replace('d=mm','d=blank',$avatar);
		}
	 ?>
	<img src="<?php echo $avatar; ?>" class="avatar"<?php if( $system->konfig->get( 'enable_schema' ) ) echo ' itemprop="image"'; ?> />
	<?php if( $system->config->get( 'layout_avatar_integration' ) == 'gravatar' || !$row->author->guest ) { ?>
		</a>
	<?php } ?>
</div>
