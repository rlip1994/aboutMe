...
$document->addScriptdeclaration('(function($){$(document).ready(function(){v=$(\'#joomcommentform\').validate({wrapper:\'mark\',submitHandler:function(a){joom_validatecomment()}});$(\'.btn[type=reset]\').click(function(){v.resetForm()})})})(jQuery);');
echo $this->loadTemplate('header'); ?>
<?php if($this->_config->get('jg_showdetailtitle') == 1): ?>
  <header class="page_header">
  <?php echo '<'. $template->params->get('itemItemHeading', 'h3') .' class="item_title" id="jg_photo_title">'; ?>
    <?php echo $this->escape($this->image->imgtitle); ?>
  <?php echo '</'. $template->params->get('itemItemHeading', 'h3') .'>'; ?>
  </header>
<?php endif;
...
<?php endif; ?>
      <img src="<?php echo $this->image->img_src; ?>" class="jg_photo_custom" id="jg_photo_big" width="<?php echo $this->image->width; ?>" height="<?php echo $this->image->height; ?>" alt="<?php echo $this->image->imgtitle;?>" <?php echo $this->extra; ?> />
...
<?php $document->addScriptdeclaration('document.getElementById(\'jg_displaynone\').className = \'jg_detailnavislide\';document.getElementById(\'jg_detailnavislide\').className = \'jg_displaynone\';');
endif;
if($this->params->get('show_zoom_icon') == 1 || $this->params->get('show_zoom_icon') == -1 || $this->params->get('show_download_icon') == 1 || $this->params->get('show_download_icon') == -1 || $this->params->get('show_nametag_icon') == 1 || $this->params->get('show_nametag_icon') == 2 || $this->params->get('show_nametag_icon') == 3 || $this->params->get('show_nametag_icon') == -1 || $this->params->get('show_favourites_icon') == 1 || $this->params->get('show_favourites_icon') == -1 || $this->params->get('show_favourites_icon') == 2 || $this->params->get('show_favourites_icon') == -2 || $this->params->get('show_report_icon') == 1 || $this->params->get('show_report_icon') == -1 || $this->params->get('show_edit_icon') || $this->params->get('show_delete_icon') || $this->event->icons){ ?>
...
<?php }
      if($this->params->get('show_edit_icon')){ ?>
...
<?php   endif; ?>
      <div id="jg_geomap">
        <?php $document->addScriptdeclaration('
          ...
          var latlng = new google.maps.LatLng('.$this->mapdata.');
          var myOptions = {
            zoom: 13,
            center: latlng,
            scaleControl: true,
...
<?php endif;
  ...
  <div class="jg_commentsarea_custom">
    <div class="jg-details-header">
      <<?php echo $template->params->get('blogPageHeading'); ?> class="jg_comments_title"><?php echo JText::_('COM_JOOMGALLERY_DETAIL_COMMENTS_EXISTING'); ?></<?php echo $template->params->get('blogPageHeading'); ?>>
      </h4>
    </div>
<?php   if(!empty($this->slider)): ?>
    <div <?php echo $this->slider; ?>>
<?php   endif;
      ...
      <form name="commentform" id="joomcommentform" action="<?php echo JRoute::_('index.php?task=comments.comment&id='.$this->image->id); ?>" method="post">
          <?php if($this->_user->get('username')): ?><span class="username"><?php echo $this->_user->get('username'); ?></span><?php endif;?>
<?php     if(!$this->_user->get('id') && $this->_config->get('jg_namedanoncomment')): ?>
          <div class="control-group"><input required type="text" class="inputbox" name="cmtname" placeholder="<?php echo $this->_mainframe->getUserState('joom.comments.name', JText::_('COM_JOOMGALLERY_COMMON_GUEST')); ?>" tabindex="1" /></div>
<?php     endif;
...
<?php     $rows = 4;
          if($this->_config->get('jg_smiliesupport')):
            ...
            <div class="control-group"><textarea required cols="40" rows="<?php echo $rows; ?>" name="cmttext" class="inputbox" tabindex="100" placeholder="<?php echo JText::_('COM_JOOMGALLERY_COMMON_COMMENT'); ?>"><?php echo $this->_mainframe->getUserState('joom.comments.text'); ?></textarea></div>
        <?php echo $this->event->captchas; ?>
        <div class="btn-group">
          <button type="submit" name="send" value="<?php echo JText::_('COM_JOOMGALLERY_DETAIL_COMMENTS_SEND_COMMENT'); ?>" class="btn btn-primary"><?php echo JText::_('COM_JOOMGALLERY_DETAIL_COMMENTS_SEND_COMMENT'); ?></button>
            &nbsp;
          <!--<button type="reset" value="<?php echo JText::_('COM_JOOMGALLERY_COMMON_DELETE'); ?>" name="reset" class="btn btn-primary"><?php echo JText::_('COM_JOOMGALLERY_COMMON_DELETE'); ?></button>-->
          </div>
      </form>
<?php   endif;
        if(!$this->params->get('commenting_allowed')): ?>
...
<?php endif;
  ...
  <?php $document->addScriptdeclaration('var jg_nameshields_width = '.$this->_config->get("jg_nameshields_width").';');
if($this->params->get('show_movable_nametag')){
    $document->addScriptdeclaration('document.id(\'jg-movable-nametag\').makeDraggable({container:\'jg_photo_big\',onComplete:function(){document.id(\'jg-movable-nametag-icon\').position({relativeTo:\'jg-movable-nametag\',position:\'upperRight\',edge:\'bottomLeft\'}).reveal()}});');
}
    $document->addScriptdeclaration('window.addEvent(\'domready\', function(){$$(\'.jg-nametag-remove-icon\').cloneEvents(document.id(\'jg-tooltip-helper\'))});');
endif;
